...
def test_rectangle_spatial_mask():
    ...
    ds = xr.Dataset(
        ...
        data_vars=dict(
            temperature=(["lon", "lat"], np.arange(4*5).reshape(4,5),),
        ),
        attrs=
            dict(description="Sample dataset",)
    )
    ...
    result_dict_bounds = rectangle_spatial_mask(
        ds = ds, 
        area = area_dict,
        include_boundary = True
        )
    result_dict_nobounds = rectangle_spatial_mask(
        ds = ds, 
        area = area_dict,
    ...
    # Call the select_region function
    result_list_bounds = rectangle_spatial_mask(
        ds = ds, 
        area = area_list,
        include_boundary = True
        )
    ...
    np.testing.assert_equal(
        ...
        sorted(ds.lon.dims + ds.lat.dims)
    )

    # Check if the result are the expected values
    # Include boundary
    np.testing.assert_array_equal(
        result_dict_bounds.values,
        should_bounds
    )
    # Do not include boundary
    np.testing.assert_array_equal(
        result_dict_nobounds.values,
        should_nobounds
    )
# %%
