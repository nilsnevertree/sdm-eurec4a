...
# Understand potential tempreateure
...
diff = (
    ds_dropsondes["potential_temperature_recon"].sel(ref_pressure=100000)
    - ds_dropsondes["potential_temperature"]
)
for t in ds_dropsondes["time"]:
    plt.plot(
        diff.sel(time=t),
        ds_dropsondes["alt"],
        color="k",
...
plt.xlabel("Difference Mearsured and reconstructed pot. Temp [K]")
...
plt.plot(
    ds_dropsondes["potential_temperature_recon"].sel(ref_pressure=100000).T,
    ds_dropsondes["alt"],
    color="r",
    linestyle="--",
    label="recon",
...
das_fit = []
for var in thermo_fit:
    ds = thermo_fit[var].eval_func(ds_dropsondes["alt"])
    ds = ds.rename(var)
    das_fit.append(ds)

ds_fit = xr.merge(das_fit)
ds_recon = ds_fit.copy()
ds_recon["relative_humidity"] = conversions.relative_humidity_from_tps(
    pressure=ds_fit["pressure"],
    temperature=ds_fit["air_temperature"],
    specific_humidity=ds_fit["specific_humidity"],
)
ref_pressure = np.arange(99500, 101500, 500)
pressure_reference = xr.DataArray(
    ref_pressure, dims=["ref_pressure"], coords={"ref_pressure": ref_pressure}
)

ds_recon["potential_temperature"] = conversions.potential_temperature_from_tp(
    pressure=ds_fit["pressure"],
    air_temperature=ds_fit["air_temperature"],
    pressure_reference=pressure_reference.sel(ref_pressure=100000, method="nearest"),
)
ds_recon["potential_temperature_full"] = conversions.potential_temperature_from_tp(
    pressure=ds_fit["pressure"],
    air_temperature=ds_fit["air_temperature"],
...
fig, ax = visulization.plot_thermodynamics(
    fig=fig,
    axs=axs,
    drop_sondes=ds_dropsondes,
    default_colors=["gray", "gray", "gray", "gray", "gray"],
)
...
fig, ax = visulization.plot_thermodynamics(
    ...
    axs=axs,
    drop_sondes=ds_fit,
    default_colors=["red", "red", "red", "red", "red"],
    plot_kwargs=dict(linestyle="-", linewidth=2),
)

fig, ax = visulization.plot_thermodynamics(
    fig=fig,
    axs=axs,
    drop_sondes=ds_recon,
    default_colors=["k", "k", "k", "k", "k"],
    plot_kwargs=dict(linestyle="-", linewidth=2),
)

axs[1, 1].plot(
    ds_recon["potential_temperature_full"],
    ds_recon["alt"],
    color="k",
    linestyle=":",
    linewidth=1,
...
for var in ["potential_temperature"]:
    logging.info(f"Fitting {var}")
    print(
...
data = ds_dropsondes["air_temperature"].sel(alt=slice(0, 1100))
plt.plot(
    data.T,
    data["alt"],
    color="k",
...
plt.plot(
    air_temperature.eval_func(data["alt"]),
    data["alt"],
    color="b",
    label="fit",
)
